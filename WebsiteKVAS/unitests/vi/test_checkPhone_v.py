...
def check_phone(phone):#finction of check phone
    compil=re.compile(r'(\+7)(\([0-9]{3}\)\s([0-9]{3}))(-[0-9]{2})(-[0-9]{2})$', re.X)#создаем регул€рное выражение
    result=compil.search(phone)#провер€ем введенную почту
    if result:#если почта верна€
        return 1
    else:
        return 0

class Test_test_зрщту(unittest.TestCase):
